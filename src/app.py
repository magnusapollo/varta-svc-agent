...
async def chat_stream(req: ChatRequest = Body(...)):
    async def event_gen() -> AsyncGenerator[bytes, None]:
        ...
        results = await retrieve_docs(plan)
        answer, citation_ids = synthesize_answer(req.message, results, model_name=settings.MODEL_NAME, max_tokens=settings.MAX_TOKENS, temperature=settings.TEMPERATURE)
